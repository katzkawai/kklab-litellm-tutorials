...
\node[sublayer, fill=red!10, draw=red!50,
      below left=0.7cm and 0.15cm of litellm.south, anchor=north east]
  (commercial) {%
    \textbf{商用 API}\\[4pt]
    OpenAI\\ Anthropic\\ Google\\ Mistral AI};

\node[sublayer, fill=green!10, draw=green!50,
      below=0.7cm of litellm.south, anchor=north]
  (managed) {%
    \textbf{マネージド推論}\\
    \textbf{プラットフォーム}\\[6pt]
...
\node[sublayer, fill=purple!10, draw=purple!50,
      below right=0.7cm and 0.15cm of litellm.south, anchor=north west]
  (selfhost) {%
    \textbf{セルフホスト推論}\\
    \textbf{エンジン}\\[4pt]
...
% --- 注釈 ---
\node[font=\scriptsize\itshape, text=gray, right=0.15cm of hw.east]
  {セルフホストの場合のみ};
...
\toprule
エンジン & GPU 要否 & LiteLLM プレフィックス & 主な用途 & 特徴 \\
\midrule
vLLM        & 必須   & \code{hosted\_vllm/} & 本番環境   & PagedAttention による高スループット \\
Ollama      & 不要\newline（推奨） & \code{ollama\_chat/} & 開発・実験 & ワンコマンドで導入、CPU でも動作 \\
llama.cpp   & 不要   & \code{hosted\_vllm/}\newline（OpenAI 互換） & 軽量推論   & C/C++ 実装、量子化モデル対応 \\
TGI         & 必須   & \code{huggingface/tgi} & 本番環境   & Hugging Face 公式、Docker ベース \\
\bottomrule
\end{tabular}
\end{table}
...
Ollama はデフォルトで \code{http://localhost:11434} に OpenAI 互換の API サーバーを起動する。
...
\code{ollama/} はテキスト補完 API を、\code{ollama\_chat/} はチャット補完 API を使用する。
会話形式で使う場合は \code{ollama\_chat/} の使用が推奨される。
\end{notebox}
...
\begin{tipbox}[ポイント]
いずれのローカル推論エンジンも OpenAI 互換の API を提供するため、LiteLLM からは \code{model} と \code{api\_base} を変更するだけで透過的に切り替えられる。
...
Together AI\footnote{\url{https://www.together.ai/}} は、200以上のオープンソースモデルをクラウド API として提供するプラットフォームである。
...
商用 API（OpenAI 等）と比較して、同等クラスのモデルをより低コストで利用できることが多い。
...
コスト    & 従量課金（低価格）    & GPU 費用のみ & GPU/CPU 費用のみ & 従量課金（高価格） \\
スループット & 高い              & 高い         & 中程度        & 高い \\
\bottomrule
\end{tabular}
\end{table}
