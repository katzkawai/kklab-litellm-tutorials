...
\subsection{アーキテクチャの全体像}

LiteLLM を中心とした LLM 活用のアーキテクチャを図~\ref{fig:architecture}に示す。
LiteLLM は API 抽象化層として、その下にある複数のバックエンド --- 商用 API、マネージド推論プラットフォーム、セルフホスト推論エンジン --- を統一的に扱う。
...
\begin{tikzpicture}[
  ...
  layer/.style={
    draw, rounded corners=3pt, minimum height=1.1cm,
    text width=14cm, align=center, font=\small,
  },
  sublayer/.style={
    draw, rounded corners=2pt, minimum height=3.0cm,
...
% --- アプリケーション層 ---
\node[layer, fill=orange!15, draw=orange!60]
  (app) {\textbf{アプリケーション層}\\[2pt]
  LangChain\enspace/\enspace 自作アプリケーション\enspace/\enspace RAG パイプライン};

% --- API 抽象化層 ---
\node[layer, fill=blue!15, draw=blue!60, below=0.5cm of app]
  (litellm) {\textbf{API 抽象化層 --- LiteLLM}\\[2pt]
  \texttt{completion()}\enspace/\enspace Router\enspace/\enspace Proxy Server};

% --- 3つの並列バックエンド（中央を先に配置し、左右を間隔指定） ---
\node[sublayer, fill=green!10, draw=green!50,
      below=1.0cm of litellm.south, anchor=north]
  (managed) {%
    \textbf{マネージド推論}\\
    \textbf{プラットフォーム}\\[6pt]
    Together AI\enspace/\enspace Groq\\
    AWS Bedrock\\
    Azure OpenAI};

\node[sublayer, fill=red!10, draw=red!50,
      left=0.4cm of managed]
  (commercial) {%
    \textbf{商用 API}\\[4pt]
    OpenAI\\ Anthropic\\ Google\\ Mistral AI};

\node[sublayer, fill=purple!10, draw=purple!50,
      right=0.4cm of managed]
  (selfhost) {%
    \textbf{セルフホスト推論}\\
    \textbf{エンジン}\\[4pt]
    vLLM\enspace/\enspace Ollama\\
    llama.cpp\enspace/\enspace TGI};

% --- ハードウェア層（セルフホストのみ） ---
\node[hwlayer, fill=gray!15, draw=gray!60,
      text width=4.2cm,
      below=0.6cm of selfhost]
  (hw) {\textbf{ハードウェア層}\\[2pt]
  GPU (CUDA)\enspace/\enspace CPU};

% --- 矢印 ---
\draw[arr] (app) -- (litellm);
\draw[arr] (litellm.south) -- ++(0,-0.25) -| (commercial.north);
\draw[arr] (litellm.south) -- (managed.north);
\draw[arr] (litellm.south) -- ++(0,-0.25) -| (selfhost.north);
\draw[arr] (selfhost) -- (hw);

% --- 注釈 ---
\node[font=\scriptsize\itshape, text=gray, below=0.15cm of hw.south]
  {セルフホストの場合のみ};

\end{tikzpicture}
\caption{LiteLLM を中心とした LLM 活用のアーキテクチャ}
...
\begin{itemize}
  \item \textbf{LiteLLM は API 抽象化層}に位置し、下層のバックエンドの違いを吸収する
  \item LiteLLM の下層は\textbf{3つの並列な経路}に分かれる:
    \begin{enumerate}
      \item \textbf{商用 API}（OpenAI, Anthropic 等）--- プロプライエタリモデルをクラウド経由で利用
      \item \textbf{マネージド推論プラットフォーム}（Together AI, Groq 等）--- オープンソースモデルをクラウド経由で利用
      \item \textbf{セルフホスト推論エンジン}（vLLM, Ollama 等）--- オープンソースモデルを自前の GPU/CPU で実行
    \end{enumerate}
  \item \textbf{ハードウェア層が必要なのはセルフホストの場合のみ}。商用 API やマネージドプラットフォームではハードウェアを意識する必要がない
  \item \textbf{アプリケーション層}（LangChain 等）は LiteLLM の上に構築することも、LiteLLM を経由せず直接各プロバイダーに接続することも可能である
\end{itemize}

\subsection{本チュートリアルについて}

本チュートリアルでは、LiteLLM の基本的な使い方から応用的な機能まで、サンプルスクリプトを交えて解説する。
各スクリプトは \code{uv} コマンドで直接実行できるように、PEP 723 のインラインメタデータ（先頭の \code{\# /// script} ブロック）を記述している。
...
Ollama       & \code{ollama/}, \code{ollama\_chat/} & llama3, phi3 等（ローカル） \\
\bottomrule
\end{tabular}
\end{table}
...
各プロバイダーの API キーを環境変数に設定する。クラウド API を使う例だけを実行する場合は、必要なプロバイダーのキーだけ設定すればよい。vLLM や Ollama のローカル実行例では API キーは不要である。
...
対応モデルでは、Pydantic モデルを \code{response\_format} に渡すことで、より厳密なスキーマに基づいた構造化出力が得られる。
...
\begin{notebox}[注意]
Pydantic ベースの厳密な構造化出力は、モデル・プロバイダー側のサポート状況に依存する。対応していない場合は JSON モードや通常のテキスト出力に切り替える。
\end{notebox}
...
この例では、同じエイリアス \code{my-gpt} に複数デプロイメントを登録し、\code{simple-shuffle} でリクエストを分散している。
フォールバックの明示的な設定は、次節の専用例で扱う。
...
ここでの \code{os.environ/...} は、Proxy 起動時に対応する環境変数を参照する記法である。
...
このスクリプトでは、ローカル側を \code{my-llm}、クラウド側を \code{my-llm-cloud} として別エイリアスで定義し、\code{fallbacks} で明示的につないでいる。
これにより、「通常はローカル、失敗時のみクラウド」という挙動が分かりやすくなる。
...
LiteLLM Proxy Server の設定ファイルに vLLM サーバーを追加することで、チーム全体でローカルモデルとクラウドモデルを同じ Proxy から利用できる。
以下は両者を明示的に登録する最小構成例である。
...
必要に応じて、Router や Proxy 側の設定でクラウドモデルをフォールバック先として組み込める。
...
Ollama\footnote{\url{https://ollama.com/}} は、ローカル環境で LLM を手軽に実行するためのツールである。
vLLM が高スループットの本番運用を志向するのに対し、Ollama は\textbf{簡便さ}を重視した設計となっている。
ワンコマンドでモデルのダウンロードと推論サーバーの起動が完了し、GPU がない環境でも CPU で動作する。

\subsection{ローカル推論エンジンの比較}

LiteLLM から利用可能な主なローカル推論エンジンを表~\ref{tab:local_engines}に比較する。

\begin{table}[htbp]
\centering
\caption{ローカル推論エンジンの比較}
\label{tab:local_engines}
\small
\begin{tabular}{lp{2cm}p{2.2cm}p{2cm}p{4cm}}
\toprule
エンジン & GPU 要否 & LiteLLM 接続方法 & 主な用途 & 特徴 \\
\midrule
vLLM        & 必須   & \code{hosted\_vllm/} & 本番環境   & PagedAttention による高スループット \\
Ollama      & 不要\newline（推奨） & \code{ollama/} または\newline \code{ollama\_chat/} & 開発・実験 & ワンコマンドで導入、CPU でも動作 \\
llama.cpp   & 不要   & OpenAI 互換 endpoint\newline + \code{api\_base} & 軽量推論   & C/C++ 実装、量子化モデル対応 \\
TGI         & 必須   & \code{huggingface/}\newline \code{tgi} & 本番環境   & Hugging Face 公式、Docker ベース \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Ollama のインストールとモデル取得}

\begin{lstlisting}[style=bash]
# Ollama のインストール（Linux）
curl -fsSL https://ollama.com/install.sh | sh

# モデルの取得（ダウンロードとサーバー起動が自動で行われる）
ollama pull llama3.2:3b

# ツール呼び出し対応の大きめのモデル
ollama pull llama3.1:8b

# サーバーの手動起動（通常は自動起動される）
ollama serve
\end{lstlisting}

Ollama はデフォルトで \code{http://localhost:11434} で待ち受ける。LiteLLM からはこの URL を \code{api\_base} に指定して利用できる。

\subsection{基本的な呼び出し}
\label{subsec:ollama_basic}

Ollama で動作するモデルを LiteLLM から呼び出すには、モデル名に \code{ollama\_chat/} プレフィックスを付け、\code{api\_base} に Ollama サーバーの URL を指定する。

\lstinputlisting[style=python, caption={Ollama による基本呼び出し（\code{scripts/22\_ollama\_basic.py}）}]{scripts/22_ollama_basic.py}

\begin{notebox}[ollama/ と ollama\_chat/ の違い]
\code{ollama/} を使うシンプルな例もあるが、本チュートリアルのサンプルは Chat Completions 形式に揃えるため \code{ollama\_chat/} を用いている。
会話形式のサンプルを統一的に扱いたい場合は \code{ollama\_chat/} が分かりやすい。
\end{notebox}

\begin{lstlisting}[style=bash]
uv run scripts/22_ollama_basic.py
\end{lstlisting}

\subsection{ストリーミング}
\label{subsec:ollama_streaming}

Ollama からのストリーミング応答も、クラウド API や vLLM と同様に \code{stream=True} で利用できる。

\lstinputlisting[style=python, caption={Ollama ストリーミング（\code{scripts/23\_ollama\_streaming.py}）}]{scripts/23_ollama_streaming.py}

\begin{lstlisting}[style=bash]
uv run scripts/23_ollama_streaming.py
\end{lstlisting}

\subsection{JSON モード}
\label{subsec:ollama_json}

Ollama では \code{format="json"} パラメータにより、JSON 形式での応答を強制できる。

\lstinputlisting[style=python, caption={Ollama での JSON モード（\code{scripts/24\_ollama\_json.py}）}]{scripts/24_ollama_json.py}

\begin{lstlisting}[style=bash]
uv run scripts/24_ollama_json.py
\end{lstlisting}

\subsection{ツール呼び出し}
\label{subsec:ollama_tool}

Llama 3.1 以降など、ツール呼び出し（Function Calling）に対応したモデルでは、Ollama 経由でもツール機能を利用できる。

\lstinputlisting[style=python, caption={Ollama でのツール呼び出し（\code{scripts/25\_ollama\_tool\_calling.py}）}]{scripts/25_ollama_tool_calling.py}

\begin{lstlisting}[style=bash]
uv run scripts/25_ollama_tool_calling.py
\end{lstlisting}

\subsection{ローカルとクラウドの応答比較}
\label{subsec:local_cloud_compare}

LiteLLM の統一インターフェースを活用すれば、ローカルモデル（Ollama）とクラウド API（OpenAI 等）の応答を同じコードで並列比較できる。
モデル選定や性能評価の際に有用である。

\lstinputlisting[style=python, caption={ローカルとクラウドの比較（\code{scripts/26\_local\_cloud\_compare.py}）}]{scripts/26_local_cloud_compare.py}

\begin{lstlisting}[style=bash]
uv run scripts/26_local_cloud_compare.py
\end{lstlisting}

\subsection{Proxy Server での Ollama 統合}

LiteLLM Proxy Server に Ollama モデルを追加する設定例を示す。

\begin{lstlisting}[style=python, caption={Ollama を含む Proxy Server 設定（\code{config.yaml}）}]
model_list:
  # Ollama ローカルモデル
...
\subsection{推論エンジン選択の指針}

どのローカル推論エンジンを選ぶかは、用途と環境に応じて判断する。

\begin{itemize}
  \item \textbf{手軽に試したい・GPU がない}: Ollama が最適。インストールからモデル実行まで数分で完了する
  \item \textbf{高スループットが必要・本番運用}: vLLM を推奨。PagedAttention による効率的な GPU メモリ管理で、多数の同時リクエストを処理できる
  \item \textbf{CPU のみで量子化モデルを動かしたい}: llama.cpp（\code{llama-server}）が適する。GGUF 形式の量子化モデルを効率的に実行できる
  \item \textbf{Hugging Face エコシステムとの統合}: TGI（Text Generation Inference）を推奨。Docker ベースで Inference Endpoints とも連携できる
\end{itemize}

\begin{tipbox}[ポイント]
これらの推論エンジンは LiteLLM から接続可能な API / エンドポイントを提供できるため、アプリケーション側では \code{model} や \code{api\_base} などの接続設定を切り替えるだけで比較・置換しやすい。
これは LiteLLM の統一インターフェースの大きな利点であり、推論エンジンの選択をアプリケーションコードから分離できる。
\end{tipbox}


% ---------------------------------------------------------
\section{Together AI との連携}
\label{sec:together}

Together AI\footnote{\url{https://www.together.ai/}} は、多数のオープンソースモデルをクラウド API として提供するプラットフォームである。
Llama、DeepSeek、Qwen、Mistral など主要なオープンソースモデルに、高速なサーバーレス推論でアクセスできる。

vLLM や Ollama が「自前の GPU でモデルを動かす」のに対し、Together AI は\textbf{GPU を持たなくてもオープンソースモデルをクラウド経由で利用できる}点が大きな違いである。
商用 API（OpenAI 等）と比較して、同等クラスのモデルをより低コストで利用できることがある。

\subsection{オープンソースモデルの利用形態}

オープンソースモデルの利用形態を表~\ref{tab:oss_deploy}に整理する。

\begin{table}[htbp]
\centering
\caption{オープンソースモデルの利用形態と比較}
\label{tab:oss_deploy}
\begin{tabular}{lp{2.5cm}p{2.5cm}p{2.5cm}p{2.5cm}}
\toprule
         & \textbf{Together AI} & \textbf{vLLM} & \textbf{Ollama} & \textbf{商用 API} \\
\midrule
デプロイ  & クラウド（マネージド） & セルフホスト & セルフホスト & クラウド（マネージド） \\
GPU      & 不要                & 必須         & 不要（推奨） & 不要 \\
対象モデル & OSS モデル          & OSS モデル   & OSS モデル   & 商用モデル \\
コスト    & 従量課金            & GPU 費用のみ & GPU/CPU 費用のみ & 従量課金 \\
スループット & 高い              & 高い         & 中程度        & 高い \\
\bottomrule
\end{tabular}
\end{table}
...
Together AI の利用には、API キーの取得と環境変数の設定が必要である。
API キーは Together AI のダッシュボード\footnote{\url{https://api.together.xyz/settings/api-keys}}から取得できる。
...
\subsection{基本的な呼び出し}
\label{subsec:together_basic}

Together AI 上のモデルを呼び出すには、\code{together\_ai/} プレフィックスにモデル ID を続ける。

\lstinputlisting[style=python, caption={Together AI の基本呼び出し（\code{scripts/27\_together\_basic.py}）}]{scripts/27_together_basic.py}

\begin{lstlisting}[style=bash]
uv run scripts/27_together_basic.py
\end{lstlisting}

\subsection{複数モデルの比較}
\label{subsec:together_multi}

Together AI の強みの一つは、多数のオープンソースモデルを同一プラットフォーム上で比較できることである。
以下のスクリプトでは、Llama、Qwen、Mistral の各モデルに同じ質問を並列で送信し、応答を比較する。

\lstinputlisting[style=python, caption={Together AI 上の複数モデル比較（\code{scripts/28\_together\_multi\_model.py}）}]{scripts/28_together_multi_model.py}

\begin{lstlisting}[style=bash]
uv run scripts/28_together_multi_model.py
\end{lstlisting}

\subsection{ストリーミング}
\label{subsec:together_streaming}

Together AI からのストリーミング応答も、他のプロバイダーと同様に \code{stream=True} で利用できる。

\lstinputlisting[style=python, caption={Together AI ストリーミング（\code{scripts/29\_together\_streaming.py}）}]{scripts/29_together_streaming.py}

\begin{lstlisting}[style=bash]
uv run scripts/29_together_streaming.py
\end{lstlisting}

\subsection{埋め込み}
\label{subsec:together_embedding}

Together AI は埋め込みモデルも提供している。
\code{embedding()} 関数で Together AI 上のモデルを指定すれば、テキストの埋め込みベクトルを生成できる。

\lstinputlisting[style=python, caption={Together AI による埋め込み生成（\code{scripts/30\_together\_embedding.py}）}]{scripts/30_together_embedding.py}

\begin{lstlisting}[style=bash]
uv run scripts/30_together_embedding.py
\end{lstlisting}

\subsection{コスト比較}
\label{subsec:together_cost}

LiteLLM のコスト追跡機能を使って、Together AI（オープンソースモデル）と商用 API（OpenAI）のコストを比較できる。
一般に、Together AI で提供されるオープンソースモデルは商用 API よりも低コストで利用できる。

\lstinputlisting[style=python, caption={Together AI とクラウド API のコスト比較（\code{scripts/31\_together\_cost\_compare.py}）}]{scripts/31_together_cost_compare.py}

\begin{lstlisting}[style=bash]
uv run scripts/31_together_cost_compare.py
\end{lstlisting}

\subsection{主な対応モデル}

Together AI 上で LiteLLM から利用できる代表的なモデルを表~\ref{tab:together_models}に示す。

\begin{table}[htbp]
\centering
\caption{Together AI の代表的モデル}
\label{tab:together_models}
\small
\begin{tabular}{lp{5cm}l}
\toprule
モデル ID & 特徴 & カテゴリ \\
\midrule
\code{meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo}  & Meta 製、高速推論版           & チャット \\
\code{meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo} & Meta 製、高性能版             & チャット \\
\code{deepseek-ai/DeepSeek-V3}                       & DeepSeek MoE、低コスト        & チャット \\
\code{deepseek-ai/DeepSeek-R1}                       & 推論特化（Reasoning）          & 推論 \\
\code{Qwen/Qwen2.5-7B-Instruct-Turbo}               & Alibaba 製、多言語対応         & チャット \\
\code{mistralai/Mistral-Small-24B-Instruct-2501}     & Mistral AI 製                & チャット \\
\code{intfloat/multilingual-e5-large-instruct}       & 多言語埋め込み、1024次元       & 埋め込み \\
\bottomrule
\end{tabular}
\end{table}

\begin{tipbox}[ポイント]
Together AI は「GPU を持っていないが、オープンソースモデルの性能を評価・活用したい」という場面に最適である。
特に研究において、Llama, DeepSeek, Qwen 等の複数のオープンソースモデルを同一条件で比較評価したい場合、Together AI + LiteLLM の組み合わせにより、最小限のコードで効率的にベンチマーク実験を実施できる。
\end{tipbox}
...
\code{22\_ollama\_basic.py}        & Ollama 基本呼び出し          & \ref{subsec:ollama_basic} \\
\code{23\_ollama\_streaming.py}    & Ollama ストリーミング         & \ref{subsec:ollama_streaming} \\
\code{24\_ollama\_json.py}         & Ollama JSON モード           & \ref{subsec:ollama_json} \\
\code{25\_ollama\_tool\_calling.py} & Ollama ツール呼び出し        & \ref{subsec:ollama_tool} \\
\code{26\_local\_cloud\_compare.py} & ローカル vs クラウド比較      & \ref{subsec:local_cloud_compare} \\
\code{27\_together\_basic.py}      & Together AI 基本呼び出し     & \ref{subsec:together_basic} \\
\code{28\_together\_multi\_model.py} & Together AI 複数モデル比較  & \ref{subsec:together_multi} \\
\code{29\_together\_streaming.py}  & Together AI ストリーミング    & \ref{subsec:together_streaming} \\
\code{30\_together\_embedding.py}  & Together AI 埋め込み生成      & \ref{subsec:together_embedding} \\
\code{31\_together\_cost\_compare.py} & Together AI コスト比較      & \ref{subsec:together_cost} \\
\bottomrule
\end{longtable}
...
OpenRouter\footnote{\url{https://openrouter.ai/}} は、多数の LLM モデルに単一の API エンドポイントからアクセスできる \textbf{マネージド SaaS 型}の LLM ゲートウェイである。
インフラの構築やサーバーの運用が不要で、API キーを取得するだけですぐに利用を開始できる。
料金はモデルごとに定義された従量課金制であり、インフラ管理なしで利用できる。
...
ツール & デプロイメント & 料金 \\
\midrule
LiteLLM    & セルフホスト（OSS） / Enterprise & OSS 版は無料。Enterprise は要問い合わせ。セルフホストのインフラ費用が別途必要 \\
OpenRouter & マネージド SaaS               & モデルごとの従量課金制。インフラ管理不要 \\
aisuite    & ライブラリ（OSS）              & 完全無料（MIT ライセンス）。各プロバイダーの API 料金のみ \\
LangChain  & ライブラリ（OSS） / LangSmith  & OSS 版は無料。LangSmith（監視・評価）は有料プランあり \\
Portkey    & クラウド / セルフホスト          & 無料プランや有料プランがあり、詳細は契約プランに依存 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{インフラ管理なしで多数のモデルにアクセスしたい}:
    OpenRouter が適する。SaaS 型のため運用負荷が低く、多数のモデルに即座にアクセスできる
...
\begin{itemize}
  ...
  \item \textbf{プロダクション対応}: Router や Proxy Server により、本番環境での運用にも対応できる
